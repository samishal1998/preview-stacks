...
| 18 | **Resolved 2026-09-19, not a leak.** `.config.aug31.yaml` is a sealed export (scrypt + AES-256-GCM) that is unreadable without the owner's passphrase. Untracked, and `.config.*.yaml` is now ignored. | — | Done |
| 19 | **Resolved 2026-09-19, not a leak.** The Traefik password in `hetzner.yml` (and in `hetzner_4.yml`, untracked since Aug 2) is a dummy example, per the owner. Untracked; a full-history scan found no API tokens or private keys. | — | Done |
| 20 | **Resolved 2026-09-19.** `.gitignore` now covers `.config.*.yaml`, `config.*.yaml` and root-level `temp*.yml`; `temp.yml` and `temp2.yml` are untracked. | — | Done |
