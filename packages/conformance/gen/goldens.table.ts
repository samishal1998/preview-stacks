...
export type Case = {
  ...
  goDivergent?: RegExp;
  /**
   * The Go transcript is a DIFFERENT document (the control Dockerfile, the cloud-init runcmd) and is
   * checked in beside the reference's as `<name>.go.json`, generated by `bun gen/goldens-go.ts` from
   * the Go binary. In go mode it is compared whole; the reference's file is the TS contract.
   */
  goGolden?: boolean;
  /** The reference's behaviour only (the 0.28.0 bridge's Go-hop refusal); skipped in go mode. */
  bunOnly?: boolean;
...
const BUN_LINES = /bun|BUN_|@samyx\/preview-stacks|unzip|install pstack |releases\/download|releases\/latest|debian:bookworm|useradd|apt-get|COPY pstack|\/usr\/local\/bin\/pstack --version|"pstack", "/;
...
export const CASES: Case[] = [
  ...
  { name: 'dockerfile', argv: ['dockerfile'], goGolden: true },
  ...
  { name: 'cloud-init-dns01-advanced-swarm', argv: [...CLOUD_INIT, '--challenge', 'dns01', '--dns-provider', 'cloudflare', '--ui', 'advanced', '--orchestrator', 'swarm', '--config-repo', 'https://github.com/example/previews.git'], env: { PSTACK_DNS_TOKEN: DNS_TOKEN }, goGolden: true },
  ...
  { name: 'upgrade-to-go-release', argv: ['upgrade', '--to', '0.29.0'], env: { PSTACK_DATA: DATA_DIR }, after: 'init-http01-basic-compose', bunOnly: true },
