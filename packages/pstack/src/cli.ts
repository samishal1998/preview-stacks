 ...
 *   pstack dockerfile   print the image Dockerfile (--ui for the SPA) — build it yourself
...
switch (args.cmd) {
  ...
  case 'dockerfile': {
    // For anyone who wants to build, tag, push or edit the image themselves — `build-image` runs
    // exactly this through `docker build -`, so what you see here is what it builds.
    const { controlDockerfile, uiDockerfile } = await import('./image.ts');
    console.log(args.uiImage ? uiDockerfile() : controlDockerfile());
    process.exit(EXIT.ok);
  }
  ...
  case 'cloud-init': {
    ...
    try {
      ...
      // Optional: most providers inject a key at boot, so demanding one here is friction. Asked
      // for, never required.
      const sshKey = args.sshKey || (args.yes ? '' : askOptional('Extra SSH public key line'));
