...
describe('pstack upgrade', () => {
  ...
  /**
   * A control directory generated by the REAL `init`, with docker faked.
   *
   * This is the test that matters. The hand-written fixtures below pin the parser's behaviour, but
   * they are only as good as my guess about what `init` emits — and that guess was wrong: the UI
   * detector looked for a service named `pstack-ui`, which exists in every generated file only as a
   * Traefik ROUTER name. Every host read as `basic`, and `upgrade` removed the advanced UI it was
   * meant to preserve, while a fabricated fixture said it worked.
   */
  const realControl = async (ui: 'basic' | 'advanced', challenge: 'http01' | 'dns01' = 'http01') => {
    const dataDir = tmpd();
  ...
  test('detects the UI mode from a control file init actually generated', async () => {
    const adv = await realControl('advanced');
    try {
      const s = await readControlState(adv.dataDir);
      // The bug: this said `basic`, so the upgrade dropped the advanced UI container.
      expect(s.ui).toBe('advanced');
      const steps = planUpgrade({ phase: 'resume', target: 'x', state: s, binPath: null });
      expect(steps.map((x) => x.cmd)).toContain('pstack build-image --ui');
      expect(steps.find((x) => x.cmd.startsWith('pstack init'))!.cmd).toContain('--ui advanced');
    } finally {
      adv.cleanup();
    }

    const basic = await realControl('basic');
    try {
      const s = await readControlState(basic.dataDir);
      expect(s.ui).toBe('basic');
      // …and a basic host does not build or wire an image it does not run.
      const steps = planUpgrade({ phase: 'resume', target: 'x', state: s, binPath: null });
      expect(steps.some((x) => x.cmd.includes('--ui'))).toBe(false);
    } finally {
      basic.cleanup();
    }
  }, 20_000);

  test('detects the challenge mode from a control file init actually generated', async () => {
    ...
    const dns = await realControl('basic', 'dns01');
    try {
      expect((await readControlState(dns.dataDir)).challenge).toBe('dns01');
    } finally {
      dns.cleanup();
    }
    const http = await realControl('advanced', 'http01');
    try {
      expect((await readControlState(http.dataDir)).challenge).toBe('http01');
    } finally {
      http.cleanup();
    }
  }, 20_000);

  test('--ui overrides the detection, for a host a bad upgrade already flipped', async () => {
    ...
    const c = await realControl('basic');
    try {
      const said: string[] = [];
      const r = await upgrade({
        ...
        phase: 'resume',
        ui: 'advanced',
        runner: createRunner({ dryRun: true, level: 'quiet' }),
        log: (l) => said.push(l),
      });
      expect(r.steps.map((x) => x.cmd)).toContain('pstack build-image --ui');
      expect(r.steps.find((x) => x.cmd.startsWith('pstack init'))!.cmd).toContain('--ui advanced');
    } finally {
      c.cleanup();
    }
  }, 20_000);
...
describe('pstack ui — switching which UI is served', () => {
  const tmpd = () =>
    `${process.env.TMPDIR ?? '/tmp'}/pstack-uiswitch-${process.pid}-${Math.trunc(performance.now() * 1000)}`;

  const realControl = async (ui: 'basic' | 'advanced') => {
    const dataDir = tmpd();
    const { init } = await import('../src/init.ts');
    await init({
      dataDir,
  ...
  test('basic → advanced builds the SPA image and re-inits with the stored token', async () => {
    const c = await realControl('basic');
    try {
      const said: string[] = [];
      const r = await switchUi({
        dataDir: c.dataDir,
        ui: 'advanced',
        runner: createRunner({ dryRun: true, level: 'quiet' }),
        log: (l) => said.push(l),
      ...
      expect(init.env?.PSTACK_TOKEN).toBeTruthy();
      // It does NOT rebuild the control image: that takes minutes and has nothing to do with which
      // UI is served.
      expect(r.steps.some((s) => s.cmd === 'pstack build-image')).toBe(false);
      expect(said.join('\n')).toContain('basic UI → advanced UI');
    } finally {
      c.cleanup();
    }
  }, 20_000);

  test('advanced → basic builds nothing — the API already carries the embedded UI', async () => {
    const c = await realControl('advanced');
    try {
      const r = await switchUi({
        dataDir: c.dataDir,
        ui: 'basic',
  ...
  test('switching to the mode already in use recreates nothing', async () => {
    ...
    try {
      ...
      expect(said.join('\n')).toContain('Already serving the advanced UI');
    } finally {
      c.cleanup();
    }
  }, 20_000);
